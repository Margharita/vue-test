<template>
  <div>
    <header class="header">
      <h1>Posts</h1>
      <h3>
        Likes: <span class="text__value">{{ $store.getters.doubleLikes }}</span>
      </h3>
      <div>
        <!-- "commit" can manipulate of state mutations -->
        <app-button @click="$store.commit('incrementLikes')">Like</app-button>
        <app-button @click="$store.commit('incrementLikes')"
          >Dislike</app-button
        >
      </div>
